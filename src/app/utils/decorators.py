...
def rate_limit(max_requests: int = 10, window: int = 60):
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            # 检查暴力破解
            if SecurityService.check_brute_force(ip, f.__name__):
                abort(429)  # Too Many Requests
            
            current_requests = cache.get(key, 0)
            if current_requests >= max_requests:
                # 记录安全事件
                SecurityService.record_security_event('rate_limit_exceeded', 
                                                 ip_address=ip, 
                                                 details={'function': f.__name__, 'requests': current_requests})
                abort(429)  # Too Many Requests
