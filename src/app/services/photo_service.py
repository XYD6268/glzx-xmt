...
class PhotoService:
    ...
    @staticmethod
    def upload_photo(file, user_id, title: Optional[str] = None, ip_address: Optional[str] = None) -> Tuple[Optional[Photo], str]:
        """
        高效照片上传处理
        """
        try:
    ...
    def _process_image_sync(photo_id):
        """同步图片处理（备用方案）"""
        try:
            from app.utils.image_utils import create_thumbnail, add_watermark
    ...
    @staticmethod
    def reject_photo(photo_id, admin_user_id, reason: Optional[str] = None) -> Tuple[bool, str]:
        """拒绝照片"""
        try:
            photo = Photo.get_by_id(photo_id)
            if not photo:
