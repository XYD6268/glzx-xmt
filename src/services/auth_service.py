...
from services.cache_service import cache, cached, CacheStrategies
...
class AuthService:
    ...
    def login_user(username: str, password: str, ip_address: str, user_agent: str = None) -> Tuple[Optional[User], str]:
        ...
        try:
            # 1. IP安全检查（优先级最高）
            if not AuthService._check_ip_security(ip_address):
            ...
            # 4. 用户级安全检查
            if not AuthService._check_user_security(user, ip_address):
                return None, '账户安全检查失败'
    ...
    def _record_successful_login(user_id, ip_address: str, user_agent: str = None):
        ...
        try:
            # 使用Celery异步处理（如果可用）
            try:
                from tasks.auth_tasks import record_login
