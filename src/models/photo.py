...
class Photo(BaseModel):
    ...
    # JSONB存储图片元数据和扩展信息
    photo_metadata = db.Column('metadata', JSONB, nullable=True)
    ...
    def search(cls, search_term: str, limit: int = 20):
        ...
        return cls.query.filter(
            ...
            cls.status == 1
        ).params(term=search_term.strip()).order_by(
            cls.vote_count.desc()
        ).limit(limit).all()
    ...
    def increment_vote_count(self, count: int = 1):
        """增加投票数"""
        self.vote_count = func.coalesce(self.vote_count, 0) + count
    
    def decrement_vote_count(self, count: int = 1):
        """减少投票数"""
        self.vote_count = func.greatest(func.coalesce(self.vote_count, 0) - count, 0)
...
class Vote(BaseModel):
    ...
    user_id = db.Column(db.UUID, db.ForeignKey('users.id'), nullable=False)
    photo_id = db.Column(db.UUID, db.ForeignKey('photos.id'), nullable=False)
