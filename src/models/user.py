...
class LoginRecord(BaseModel):
    ...
    login_time = db.Column(db.DateTime(timezone=True), server_default=db.func.now())
...
class IpBanRecord(BaseModel):
    ...
    is_active = db.Column(db.Boolean, default=True, nullable=False, index=True)
    banned_at = db.Column(db.DateTime(timezone=True), server_default=db.func.now())
...
class UserWhitelist(BaseModel):
    ...
    user_id = db.Column(db.UUID, db.ForeignKey('users.id'), nullable=False, unique=True)
    description = db.Column(db.String(200), nullable=True)
    created_by = db.Column(db.String(50), nullable=False)
