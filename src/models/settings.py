...
class Settings(BaseModel):
    ...
    # 时间设置
    vote_start_time = db.Column(db.DateTime(timezone=True), nullable=True)
    vote_end_time = db.Column(db.DateTime(timezone=True), nullable=True)
    ...
    # JSONB存储扩展设置（便于动态配置）
    extended_settings = db.Column(JSONB, nullable=True)
    ...
    def get_extended_setting(self, key: str, default=None):
        """获取扩展设置"""
        if self.extended_settings is None:
            return default
        return self.extended_settings.get(key, default)
    
    def set_extended_setting(self, key: str, value):
        """设置扩展配置"""
        if self.extended_settings is None:
            self.extended_settings = {}
        
        self.extended_settings[key] = value
        # 触发SQLAlchemy更新检测
        self.extended_settings = self.extended_settings.copy()
...
class UserAgreementRecord(BaseModel):
    ...
    agreement_id = db.Column(db.UUID, db.ForeignKey('agreements.id'), nullable=False)
    ip_address = db.Column(db.String(45), nullable=False, index=True)
    read_time = db.Column(db.Integer, nullable=False)  # 秒
    session_id = db.Column(db.String(100), nullable=True)
    agreed_at = db.Column(db.DateTime(timezone=True), server_default=db.func.now())
